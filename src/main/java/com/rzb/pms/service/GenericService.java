...
public class GenericService {
	...
	public List<GenericDto> findAllGenerics(Pageable pageable) {
		...
		List<GenericDto> genericData = data.getContent().stream().map(x -> new GenericDto(x))
	...
	public GenericDto getGenericById(String id) {
		...
		return GenericDto.builder().genericId(genData.get().getGenericId()).name(genData.get().getName())
				.drugs(genData.get().getDrugs()).build();
	...
	public GenericResponseByName getGenericByName(String name) {
		...
		if (name == null) {
			log.error("Generic name can't be null");
		...
		if (generic == null) {
			log.error("No data found for given generic name");
			throw new CustomException("No data found for given generic name", HttpStatus.NOT_FOUND);
		}

		return GenericResponseByName.builder().name(generic.getName()).genericId(generic.getGenericId()).build();
	}
}
