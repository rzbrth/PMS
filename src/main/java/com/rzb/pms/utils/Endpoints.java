...
//	public static final String SEARCH_GENERIC_BY_NAME = "/name/{name}";
	...
	public static final String DRUG = "/drug", 
			GET_DRUG_BY_GENERIC_ID = GENERIC + "/{genericId}" + DRUG,
			GET_DRUG_BY_GENERIC_NAME = GENERIC + "/{genericName}", 
			ALL_MEDECINE = "/findAll",
			SEARCH_MEDECINE_BY_ID = "/{drugId}", 
			UPDATE_DRUG_BY_ID = "/{drugId}", 
			ADD_DRUG = "/addDrug",
			GET_DRUG_BY_COMPOSITION = "/substitute/{composition}", 
			GET_DRUG_BY_NAME = "/name/{brandName}",
			RETURN_EXPIRED_ITEM = "/return", 
			FIND_ALL_RETURN_REQUEST = "/getReturnReq";
...
//	public static final String GET_MEDECINE_BY_MANUFACTURE = "/{id}" + DRUG;
	...
	public static final String STOCK = "/stock", 
			UPDATE_STOCK = "/{stockId}", 
			GET_STOCK_BY_ID = "/{stockId}",
			DELETE_STOCK = "/{stockId}", 
			GET_TOP4_ABOUT_TO_EXPIRE_STOCK = "/findAboutToExpireItem", 
			STOCK_EXPIRE = "/expireCheck";

	// Purchase Order
	public static final String PO = "/po", 
			ADD_PO = PO, 
			GET_ALL_PO_WITH_EXPORT_OPTION = PO, 
			GET_ONE = "/{poId}",
			UPDATE_PO = "/{poId}", 
			DELETE_PO = "/{poId}";
